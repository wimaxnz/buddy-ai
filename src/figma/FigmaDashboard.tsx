...
// ─── Live data helpers (values only — Figma layout unchanged) ─────────────────
...
function liveEmotionChip(live: LiveTelemetry | null): { label: string; cls: string } {
  const raw = String(live?.express_face || live?.emotion || live?.face_state || 'idle').toLowerCase()
  const map: Record<string, string> = {
    happy: 'Happy', idle: 'Idle', listening: 'Listening', thinking: 'Thinking',
    speaking: 'Speaking', curious: 'Curious', excited: 'Excited', celebrating: 'Celebrating',
    worried: 'Worried', sad: 'Sad', laughing: 'Laughing', love: 'Love', shy: 'Shy', sleeping: 'Sleeping',
  }
  const label = map[raw] || raw.charAt(0).toUpperCase() + raw.slice(1)
  const cls = ['happy', 'excited', 'celebrating', 'love', 'laughing'].includes(raw)
    ? 'chip-green'
    : ['worried', 'sad'].includes(raw) ? 'chip-yellow' : 'chip-blue'
  return { label, cls }
}

function liveTelemetryRows(live: LiveTelemetry | null) {
  ...
  return [
    { label: 'CPU Usage', base: cpu, variance: 12, unit: '%', color: 'var(--accent)', warn: 80 },
    { label: 'RAM Used', base: ram, variance: 8, unit: '%', color: 'var(--purple)', warn: 90 },
    { label: 'Temperature', base: Math.round(temp), variance: 6, unit: '°C', color: 'var(--orange)', warn: 70 },
    { label: 'Battery', base: Math.round(battery), variance: 4, unit: '%', color: 'var(--green)' },
    { label: 'WiFi RSSI', base: rssi, variance: 10, unit: 'dBm', color: 'var(--teal)' },
    { label: 'Audio Level', base: 12, variance: 22, unit: 'dB', color: 'var(--yellow)' },
  ]
}
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, height: '100%', minHeight: 0 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minHeight: 0, overflow: 'auto', paddingRight: 2 }}>
        ...
        <div style={{
          ...
          <MiniFace emotion={(live?.express_face || live?.emotion || "happy") as string} />
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              <span style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.02em' }}>Buddy</span>
              {(() => {
                const online = live?.online ?? false
                const emo = liveEmotionChip(live)
                return (
                  <>
                    <span className={`chip ${online ? 'chip-green' : 'chip-yellow'}`} style={{ fontSize: 10 }}>
                      <span style={{ width: 5, height: 5, borderRadius: '50%', background: online ? 'var(--green)' : 'var(--yellow)', display: 'inline-block' }} />
                      {online ? 'Online' : 'Offline'}
                    </span>
                    <span className={`chip ${emo.cls}`} style={{ fontSize: 10 }}>{emo.label}</span>
                  </>
                )
              })()}
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>
              {(() => {
                const who = live?.recognised_user || 'Buddy'
                const ago = formatAgo(live?.last_seen_seconds_ago)
                const room = String(settings?.location_label || settings?.room_name || 'Living Room')
                const device = live?.firmware_build || 'M5Stack CoreS3'
                const heard = live?.last_heard ? ` · Heard: "${live.last_heard}"` : ''
                return `Last seen: ${who}, ${ago} · ${room} · ${device}${heard}`
              })()}
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div style={{
            ...
            {liveTelemetryRows(live).map(row => (
              <TelemetryRow key={row.label} {...row} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minHeight: 0, overflow: 'auto' }}>
        ...
        <div style={{
          ...
          {(() => {
            const rows = [
              { label: 'Firmware', value: live?.firmware_build ? `v${live.firmware_build}` : '—', ok: !!live?.firmware_build },
              { label: 'WiFi', value: String(settings?.wifi_ssid || settings?.location_label || 'Living Room'), ok: live?.online ?? false },
              { label: 'Storage', value: '14.2 GB free', ok: true },
              { label: 'Camera', value: live?.camera_enabled ? (live?.camera_active ? 'Active' : 'Enabled') : 'Disabled', ok: !!live?.camera_enabled },
              { label: 'Face AI', value: 'Ready', ok: true },
              { label: 'Last sync', value: formatAgo(live?.last_seen_seconds_ago), ok: (live?.last_seen_seconds_ago ?? 999) < 120 },
            ]
            return rows
          })().map(row => (
...
function placeholderLiveLines(
  id: ScreenId,
  live: LiveTelemetry | null,
  settings: Record<string, unknown> | null,
  weather: { temp_display?: string; condition?: string; city?: string } | null,
): string[] {
  const s = settings || {}
  switch (id) {
    case 'camera':
      return [
        live?.camera_enabled ? 'Camera enabled on device' : 'Camera disabled',
        live?.camera_active ? `Streaming · ${live?.camera_fps_actual ?? '—'} fps` : 'Not streaming',
        live?.camera_init_ok ? 'Sensor ready' : 'Sensor initialising',
      ]
    case 'camera-preview':
      return [
        live?.camera_preview_enabled !== false && live?.camera_enabled ? 'Preview available' : 'Preview off',
        live?.camera_last_frame_bytes ? `Last frame ${live.camera_last_frame_bytes} bytes` : 'Waiting for frame',
      ]
    case 'voice':
      return [
        String(s.voice_provider || s.tts_provider || 'Edge TTS'),
        live?.last_heard ? `Last heard: "${live.last_heard}"` : 'No recent speech',
        live?.tts_ms != null ? `TTS latency ${live.tts_ms} ms` : '',
      ].filter(Boolean)
    case 'memory':
      return [
        live?.recognised_user ? `Active user: ${live.recognised_user}` : 'No recognised user',
        live?.last_reply ? `Last reply: "${String(live.last_reply).slice(0, 48)}"` : '',
      ].filter(Boolean)
    case 'stories':
      return [
        live?.story_active || live?.story_mode ? 'Story mode active' : 'Story mode idle',
        String(s.story_brain_enabled !== false ? 'Story brain enabled' : 'Story brain off'),
      ]
    case 'clock':
      return [
        String(s.clock_layout || 'classic_analog').replace(/_/g, ' '),
        s.clock_show_date !== false ? 'Date shown on face' : 'Date hidden',
      ]
    case 'weather':
      return [
        weather?.temp_display ? `${weather.temp_display} · ${weather.condition || ''}`.trim() : 'Weather loading',
        String(weather?.city || s.location_city || s.home_city || 'Location not set'),
      ]
    case 'rgb':
      return [
        `Profile: ${live?.express_profile || '—'}`,
        `Face: ${live?.express_face || live?.emotion || 'idle'}`,
        live?.express_rgb_bright != null ? `Brightness ${live.express_rgb_bright}` : '',
      ].filter(Boolean)
    case 'developer':
      return [
        live?.firmware_build ? `Firmware v${live.firmware_build}` : 'Firmware —',
        `STT ${live?.stt_ms ?? 0} / LLM ${live?.llm_ms ?? 0} / TTS ${live?.tts_ms ?? 0} ms`,
        live?.free_heap != null ? `Heap ${live.free_heap}` : '',
      ].filter(Boolean)
    case 'device-status':
      return [
        live?.online ? 'Device online' : 'Device offline',
        formatAgo(live?.last_seen_seconds_ago),
        live?.base_battery_pct != null ? `Battery ${live.base_battery_pct}%` : '',
      ].filter(Boolean)
    case 'telemetry':
      return liveTelemetryRows(live).slice(0, 4).map(r => `${r.label}: ${r.base}${r.unit}`)
    case 'privacy':
      return [
        live?.camera_enabled ? 'Camera on' : 'Camera off',
        s.face_recognition_enabled ? 'Face recognition enabled' : 'Face recognition off',
      ]
    case 'security':
      return [
        s.dashboard_password ? 'Dashboard password set' : 'No dashboard password',
        s.developer_password ? 'Developer lock enabled' : 'Developer lock off',
      ]
    case 'family':
      return live?.recognised_user ? [`Recognised: ${live.recognised_user}`] : ['No family member recognised yet']
    default:
      return []
  }
}

function PlaceholderScreen({ title, icon, screenId }: { title: string; icon: keyof typeof Icons; screenId?: ScreenId }) {
  const { live, settings, weather } = useLiveDataContext()
  const hints = screenId ? placeholderLiveLines(screenId, live, settings, weather) : []
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        {hints.length > 0 && (
          <div style={{ marginTop: 14, fontSize: 11, lineHeight: 1.7, color: 'var(--text-tertiary)' }}>
...
function BuddyFaceScreen() {
  ...
  }, [live?.express_face, live?.face_state, live?.emotion, live?.state, live?.speaking, live?.thinking, live?.listening, live?.story_active, live?.story_mode])

  // Arousal scale overrides eye openness when active
  const applyArousal = (baseCfg: FaceCfg): FaceCfg => {
    if (!showArousal || live) return baseCfg
    ...
    <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', letterSpacing: '0.08em' }}>
          {`320 × 240 · ${live?.firmware_build ? `v${live.firmware_build}` : 'M5Stack CoreS3'}`}
        </div>
...
function Sidebar({ active, onNavigate }: { active: ScreenId; onNavigate: (id: ScreenId) => void }) {
  const { live, settings } = useLiveDataContext()
  const online = live?.online ?? false
  const ip = String(settings?.device_ip || settings?.buddy_ip || (typeof window !== 'undefined' ? window.location.hostname : ''))
  const fw = live?.firmware_build ? `v${live.firmware_build}` : '—'
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: online ? 'var(--green)' : 'var(--yellow)', flexShrink: 0, boxShadow: online ? '0 0 6px var(--green)' : 'none' }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)' }}>{online ? 'Buddy is Online' : 'Buddy is Offline'}</div>
          <div style={{ fontSize: 10, color: 'var(--text-tertiary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ip} · {fw}</div>
        </div>
      </div>
    </aside>
  )
...
function renderScreen(id: ScreenId): React.ReactNode {
  ...
  return <FigmaScreenRouter id={id} />
}
...
function readScreenFromHash(): ScreenId {
  ...
  return SCREEN_IDS.includes(id as ScreenId) ? (id as ScreenId) : 'dashboard'
}
